...
namespace GTA {
 ...
 public enum Control {LookLeftRight,LookUpDown,MoveUpDown,MoveLeftRight,Sprint,Duck,VehicleAccelerate,VehicleBrake,FrontendLb,FrontendRb,FrontendLt,FrontendRt}
 public static class Game {public static readonly System.Collections.Generic.Dictionary<Control,float> Axes=new System.Collections.Generic.Dictionary<Control,float>();public static readonly System.Collections.Generic.HashSet<Control> Held=new System.Collections.Generic.HashSet<Control>();public static float GetControlValueNormalized(Control c)=>Axes.TryGetValue(c,out var v)?v:0f;public static float GetDisabledControlValueNormalized(Control c)=>Axes.TryGetValue(c,out var d)?d:0f;public static bool IsControlPressed(Control c)=>Held.Contains(c);public static float LastFrameTime=.033f;public static bool IsPaused;public static int GenerateHash(string s)=>s.GetHashCode();public static int GameTime; public static float TimeScale=1; public static Player Player=new Player(); public static bool Arrested;}
 ...
 public class Entity {public bool IsDead;public bool IsPersistent,IsOnScreen,Released;static int next; public int Handle=++next; public bool Present=true; public bool Exists()=>Present; public Vector3 Position,Velocity,Rotation; public float Heading; public bool IsPositionFrozen,IsCollisionEnabled=true,IsVisible=true;}
 public enum Relationship {Neutral,Hate}
 public enum WeaponHash {MicroSMG,CarbineRifle}public class Weapons {public void Give(WeaponHash w,int ammo,bool equip,bool loaded){}} public class Ped:Entity {public bool IsInWater,IsRagdoll;public VehicleSeat SeatIndex {get{if(CurrentVehicle!=null)foreach(var pair in CurrentVehicle.Seats)if(pair.Value==this)return pair.Key;return VehicleSeat.None;}}public Weapons Weapons=new Weapons();public int MaxHealth;public bool CanSufferCriticalHits;public void ClearBloodDamage(){}public void ClearLastWeaponDamage(){}public int Health,Armor,Accuracy;public bool IsAlive=>!IsDead;public void SetIntoVehicle(Vehicle v,VehicleSeat s){CurrentVehicle=v;Position=v.Position;v.Seats[s]=this;}
  public bool IsInCombat,IsShooting,IsInvincible,AlwaysKeepTask,BlockPermanentEvents,Entering,Hostile;
  public Vector3 ForwardVector; public Vehicle CurrentVehicle; public RelationshipGroup RelationshipGroup; public Ped CombatTarget;
 ...
 public class Model {public int Hash=>(Name??"").GetHashCode();public string Name;public bool IsValid=true,IsInCdImage=true;public Model(){}public Model(string s){Name=s;IsHelicopter=s=="buzzard";IsCar=!IsHelicopter;}public void MarkAsNoLongerNeeded(){}public bool IsCar=true,IsBike;public bool IsTrain,IsSubmarine,IsHelicopter,IsPlane,IsBoat;}
 public enum DrivingStyle {Normal,AvoidTraffic,AvoidTrafficExtremely} public enum VehicleMissionType {GoTo,Cruise,Circle,Attack,Flee}
 public enum HeliMissionFlags {None} public enum BoatMissionFlags {None} public enum VehicleDrivingFlags {None}
 public class Vehicle:Entity {public bool IsInvincible;public Blip Indicator;public Blip AddBlip(){return Indicator=new Blip();}public void PlaceOnGround(){}public Vector3 ForwardVector=new Vector3(0,1,0);public int PassengerCapacity=>Capacity;public bool IsSirenActive;public bool IsEngineRunning;public Model Model=new Model();public bool IsDriveable=true,IsInAir;public float HeightAboveGround;public Ped GetPedOnSeat(VehicleSeat s)=>Seats.TryGetValue(s,out var p)?p:null;public float Speed; public string DisplayName="test car"; public int Capacity=3; public Dictionary<VehicleSeat,Ped> Seats=new Dictionary<VehicleSeat,Ped>(); public float TopSpeed=60f;public HashSet<VehicleSeat> TurretSeats=new HashSet<VehicleSeat>();public bool IsSeatFree(VehicleSeat s)=>((int)s==-1||((int)s>=0&&(int)s<Capacity))&&!Seats.ContainsKey(s);}
 ...
 public class Tasks {public VehicleMissionType LastMission;public int VehicleMissions;public Ped MissionTarget;public float MissionSpeed;public int MissionFlags;public void StartVehicleMission(Vehicle v,Ped p,VehicleMissionType t,float s,VehicleDrivingFlags f,float r,float straight,bool wrong){VehicleMissions++;LastMission=t;MissionTarget=p;MissionSpeed=s;MissionFlags=(int)f;DriveKind="flee";DriveCalls++;}public void StartScenario(string s,Vector3 p,float h){}public void VehicleShootAtPed(Ped p){Shots++;LastTarget=p;}public int Shots;public void StartHeliMission(Vehicle v,Ped p,VehicleMissionType m,float s,float r,int h,int min,float a,float slow,HeliMissionFlags f){DriveKind="heli-attack";}public int Gotos;public void GoTo(Vector3 p){Gotos++;}public int Runs;public void RunTo(Vector3 p,bool a,int t){Runs++;} public void LeaveVehicle(){owner.CurrentVehicle=null;}public void VehicleChase(Ped p){}public int Wanders;public void WanderAround(Vector3 p,float r){Wanders++;}public void StandStill(int t){}
  public string DriveKind;public Vector3 DriveTarget;public int DriveCalls;public float DriveSpeed;
  public int LastDriveStyle;public void DriveTo(Vehicle v,Vector3 p,float r,float s,DrivingStyle f){DriveKind="road";DriveTarget=p;DriveSpeed=s;LastDriveStyle=(int)f;DriveCalls++;}
  public void CruiseWithVehicle(Vehicle v,float s,DrivingStyle f){DriveKind="cruise";DriveSpeed=s;LastDriveStyle=(int)f;DriveCalls++;}
 ...
 public enum BlipSprite {Standard,Tank,PoliceCarDot,PoliceHelicopter} public enum BlipColor {Yellow,Red,Blue}
 ...
 // The survey's free camera. Thin on purpose: flying it is mouse, stick and eye, and
 // a stand-in that reported a camera moving would prove nothing about how it feels.
 // What these checks are for is that the view is always given back.
 public class Camera {public Vector3 Position,Rotation;public float FieldOfView=50f;public bool IsActive,Deleted;
  public bool Exists()=>!Deleted;public void Delete(){Deleted=true;IsActive=false;}}
 public static class GameplayCamera {public static Vector3 Position,Rotation;public static float FieldOfView=50f;}
 public static class World {public static Camera RenderCam;public static Camera CreateCamera(Vector3 p,Vector3 r,float f)=>new Camera{Position=p,Rotation=r,FieldOfView=f};public static Vector3 GetSafeCoordForPed(Vector3 p,bool b,int f)=>p;public static int AddRelationshipGroup(string s)=>1;public static Ped CreatePed(Model m,Vector3 p,float h)=>new Ped{Position=p,Heading=h};public static bool SphereVisible;public static Vehicle[] NearbyVehicles=new Vehicle[0];public static List<Vehicle> Vehicles=new List<Vehicle>();public static Vehicle[] GetNearbyVehicles(Vector3 p,float r)=>NearbyVehicles;public static Func<Vector3,Vector3> StreetResolver;public static Vector3 GetNextPositionOnStreet(Vector3 p)=>StreetResolver==null?p:StreetResolver(p);public static Vehicle CreateVehicle(Model m,Vector3 p,float h){var v=new Vehicle{Position=p,Heading=h,Model=m};Vehicles.Add(v);return v;}public static Blip WaypointBlip;public static object RenderingCamera; public static Ped[] Nearby=new Ped[0];public static Blip LastBlip;public static bool CollisionReady;
...
namespace GTA.Native {
 ...
 public static class Function {
  ...
  public static T Call<T>(Hash h,params object[] args){object value=false;
   ...
   if(h==Hash.GET_PED_RELATIONSHIP_GROUP_HASH)return (T)(object)((Ped)args[0]).RelationshipGroup.Hash;
