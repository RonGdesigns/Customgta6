...
public static partial class RegressionTests {
 ...
 public static int Main(string[] args){try{IndividualHangoutChecks();NonlethalCompanionChecks();CrewOrderChecks();RacePacingChecks();RivalGridChecks();CrewDrivingChecks();PlacementEditorChecks(args[0]);MilitaryAftermathChecks();TeamCombatChecks();ActiveRecoveryChecks();MilitaryLifecycleChecks();ReunionAndChaseTests();ConvoyAndIndependentTests();CompanionTests();FriendlyFireTests();DriverTests();HandoverTests();HoldTests();ObjectiveTests();SurveyTests(args[0]);DeathTests();Console.WriteLine(checks+" checks passed (stand-ins; live GTA validation still required).");return 0;}catch(Exception ex){Console.Error.WriteLine(ex);return 1;}}
}
