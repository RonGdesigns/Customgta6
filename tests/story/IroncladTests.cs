...
public static partial class StoryTests
{
 static void IroncladChecks()
 {
  ...
  Use(crew,CrewSlot.Guess);Game.Player.Character.SetIntoVehicle(m9.Frogger,VehicleSeat.Driver);m9.Frogger.HeightAboveGround=10f;m9.Frogger.IsInAir=true;m9.Tick();Check(m9.CurrentStage==1,"Airborne, the convoy rolls and the shadow begins");
  ...
  Use(crew,CrewSlot.Guess);Game.Player.Character.SetIntoVehicle(m9.Frogger,VehicleSeat.Driver);m9.Frogger.HeightAboveGround=10f;m9.Frogger.IsInAir=true;m9.Tick();escort=m9.EscortTruck;escortDriver=World.Created.First(p=>p.CurrentVehicle==escort);
  ...
  Use(crew,CrewSlot.Guess);m9.Frogger.Position=c.Locations.Position("M09.Pickup");m9.Frogger.HeightAboveGround=0f;m9.Frogger.Speed=0f;m9.Frogger.IsInAir=false;Game.Player.Character.SetIntoVehicle(m9.Frogger,VehicleSeat.Driver);m9.Tick();
  ...
  Use(crew,CrewSlot.Guess);Game.Player.Character.SetIntoVehicle(m9.Frogger,VehicleSeat.Driver);m9.Frogger.IsInAir=false;m9.Frogger.HeightAboveGround=0f;m9.Frogger.Position=c.Locations.Position("M09.Bunker");Game.Player.Character.Position=m9.Frogger.Position;m9.Tick();c.Dialogue.Clear();m9.Tick();
  ...
  Reset();crew=Roster();c=Context(crew);c.State=CampaignState.Load(Path.Combine(root,"iron10.json"));c.State.SetCargo("turbineEngines","M08.Connector");GameUtils.RoadAvailable=true;var m10=new M10OpenThrottle();
  ...
  GTA.UI.Screen.Subtitle=null;Interact(m11,c,CrewSlot.Guess,World.Props.First(p=>p.Model.Name=="prop_tool_bench02").Position+new Vector3(1.5f,0,0),6);Check(m11.CurrentStage==1,"Mounts fabricated, Ice is called to the dyno");
  ...
  m11.Granger.CurrentRPM=0.62f;GameUtils.LastProgress=-1f;for(int i=0;i<30&&m11.CurrentStage==2;i++){GTA.Native.Function.Held[(Control)71]=i<6;Game.GameTime+=500;m11.Tick();}
