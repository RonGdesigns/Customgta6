...
public static partial class StoryTests
{
 ...
 static void Reset(){SlowMotion.Reset();Function.ResetTimecycle();Game.IsPaused=false;World.RaycastHandler=null;World.SafeCoordHandler=null;GameUtils.FailModel=null;World.FailPropModel=null;World.FailVehicles=false;World.CurrentDate=new DateTime(2026,9,12);Function.WantedSeen=true;Function.StarsFlashing=false;Function.Paused=false;Function.ScoreAvailable=true;Function.EaseHoldsView=false;Function.ExplosiveDamage.Clear();CrewBoarding.ForgetOrders();ScriptedFlight.ResetProbes();Function.TestDamage.Clear();Function.MarineBlocked=false;Function.MarineFloor=null;Function.MarineObstruction=null;GameUtils.ClockCalls=0;GameUtils.WeatherCalls=0;Game.GameTime=100;Game.Player=new Player();Game.Accept=false;Game.Pressed.Clear();World.Created.Clear();World.Vehicles.Clear();World.Props.Clear();World.FailNavigation=false;World.FailNavigationNear=null;Function.Seabed=-40f;Function.SeabedKnown=true;World.NearbyVehicles=new Vehicle[0];World.NearbyProps=new Prop[0];World.GroundHeight=0f;World.WaterAvailable=true;World.FailPeds=false;World.RenderingCamera=null;World.FailCamera=false;World.CollisionReady=false;GameUtils.Faded=false;Function.Values.Clear();Function.Held.Clear();Function.Axes.Clear();Function.ThrowOnce=null;Function.WeaponGroup=0;Script.Waited=0;}
 ...
 static void NewMissionBehavior(){
  ...
  Use(crew,CrewSlot.Guess);chase.Speed=20f;m.Tick();Check(m.RequiredSwitch==null,"The disabled-van handoff lets Guess steer and slow down before forcing a switch");
  chase.Speed=0f;m.Tick();Check(m.RequiredSwitch==CrewSlot.Ice,"Once stopped, the mission requests Ice");
  ...
  ice.Position=chase.Position+new Vector3(3,0,0);int entries=ice.Task.Enters;Game.Pressed.Add(Control.Context);m.Tick();
  Check(m.Drives==null&&ice.Task.Enters==entries+1&&m.RequiredSwitch==null,"Ice stows the carried drives before the door and starts a normal entry animation without changing characters");
 ...
 static void Dispatch(){
  ...
  manager.Start(def);c.Cutscenes.Skip();manager.Update();Check(mission.Begins==1&&mission.Ticks==0,"Skipping briefing starts gameplay exactly once on next dispatch");manager.Update();Check(mission.Ticks==1,"Gameplay resumes normal ticking after scene");
 ...
 public static int Main(string[] args){try{dataDir=args[0];root=args[1];HostConstructorChecks();VehicleGroundChecks();InteractionAnimationChecks();SoloReturnChecks();SiteFixChecks();ActThreeRequestChecks();NonlethalChecks();BM01PlacementChecks();CrewIntentChecks();AuditActIIIAChecks();AuditActIIIBChecks();AuditActIIChecks();AuditActIChecks();PlaytestSeptember22Checks();BonusOspreyChecks();ActAuditChecks();ActThreeAuditChecks();InterfaceChecks();CrewOrderChecks();ReviewRepairChecks();SprintRouteChecks();AbilityReworkChecks();SeatingContractChecks();SurveyGuardChecks();BlackoutProtocolChecks();JudicialStrikeChecks();RedactedVaultChecks();ReturnToTheConcreteChecks();PillboxRedoubtChecks();SubterraneanSweepChecks();IronInTheDrainChecks();MazeBankChecks();VehicleSpecsChecks();VehicleStageChecks();NitrousSpeedChecks();CustomsCardChecks();PresentationRepairChecks();BoardingAndChaseChecks();PaletoInteriorChecks();PresenceAndCameraChecks();PreflightChecks();SpawnGroupChecks();SurveyReadingChecks();ScenePreviewChecks();InstrumentObjectiveChecks();VehiclePricingChecks();ScenePerformanceChecks();SurfaceAndHoldChecks();GuardAwarenessChecks();GuardAwarenessWiringChecks();SharedSystemsFoundationChecks();AtmosphereTransitionChecks();SoloDesertRepairChecks();M19FloatRepairChecks();PhoneHangoutChecks();PhoneCommerceChecks();PhoneLayoutChecks();SoloHarborFollowupChecks();PhoneOverlayChecks();HubChecks();CampaignPhoneChecks();FinalPreparationChecks();MarinePreparationChecks();PreparationChecks();Mission1218RepairChecks();Mission610RepairChecks();NitrousAndShopGarageChecks();SecondWindowSoloChecks();M05BoatDisableChecks();Mission78RepairChecks();CommerceFleetChecks();RelayFuelPackageChecks();PlacementRepairChecks();ProgressionSnapshot();M05LocationTestChecks();PlacementMissionChecks();SeptemberRepairChecks();GarageChecks();PlaythroughRepairChecks();HarborRepairChecks();SecondOperationChecks();ContinuousPortHeistChecks();MarketChecks();PowerChecks();StreetsChecks();CampaignAuditChecks();ApartmentWeaponChecks();PersonalChecks();ExpansionChecks();HomeAndVehicleChecks();ClarityMissionChecks(); CampaignFlowChecks();AssignmentAndRouteChecks();NewMissionBehavior();Switches();Scenes();M01Regression();PadSwitchTests();WheelAndLooks();NewControlsAndRoutes();AbilityBindingTests();StickTests();Dispatch();Markers();TransportAndRace();Waves();HandoffPassChecks();CorrectionPassChecks();FixBranchChecks();PackageABChecks();Round2Checks();VisualsDamageChecks();M01ExitChecks();ShieldChecks();PortHeistChecks();CrewVanChecks();StoryToPlayChecks();OpenSliceChecks();ControlChecks();RoomChecks();RelayChecks();Round3Checks();TierChecks();FoundryConvoyChecks();IroncladChecks();SoloChecks();HarborChecks();LiftChecks();HeistChecks();SageChecks();Console.WriteLine(checks+" story/runtime checks passed.");return 0;}catch(Exception ex){Console.Error.WriteLine(ex);return 1;}}
}
