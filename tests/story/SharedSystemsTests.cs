...
{
    /// <summary>
    /// These need no campaign data directory. Called from Main like every other
    /// suite rather than from a static constructor, so a failure here reports as an
    /// ordinary FAIL instead of a type-initialization error with the cause buried.
    /// </summary>
    static void SharedSystemsFoundationChecks()
    {
        ...
        // ---- The review is wired to a real chapter and cannot be the reason one fails.
        Reset();
        var crew = Roster();
        var context = Context(crew);
        context.State = CampaignState.Load(System.IO.Path.Combine(root, "doctor-review.json"));
        context.Doctor.BeginMission("M46", "Paleto Deep-Sea: Vault Crack");
        int before = context.Doctor.Entries.Count;
        Paleto.Review(context, PlacementContract.Ped("M46.Bridge"), PlacementContract.Interaction("M46.Vault"),
            PlacementContract.Ped("Nonexistent.Key"));
        Check(context.Doctor.Entries.Count == before + 3,
            "Reviewing a chapter's authored points records one named verdict each");
        Check(context.Doctor.Entries.Any(e => e.Subject == "Nonexistent.Key" && e.Severity == DiagnosticSeverity.Error),
            "A key that is not in the book is reported by name rather than passing quietly");
        var reviewed = new PlacementContract[] { null };
        Paleto.Review(context, reviewed);
        Paleto.Review(null, PlacementContract.Ped("M46.Bridge"));
        Check(true, "A diagnostic review never throws, so it can never be why a mission refuses to start");
