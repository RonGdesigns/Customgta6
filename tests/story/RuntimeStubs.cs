...
namespace GTA {
 ...
 public class Ped:Entity {public Model Model=new Model("ped");public bool IsCop;public int Health=300,MaxHealth=300;public bool CanSufferCriticalHits;public VehicleSeat SeatIndex {get {if(CurrentVehicle!=null)foreach(var p in CurrentVehicle.Seats)if(p.Value==this)return p.Key;return VehicleSeat.None;}}
 ...
 public static class World {public static Vehicle[] GetAllVehicles()=>Vehicles.ToArray();public static Prop[] GetNearbyProps(Vector3 p,float r)=>new Prop[0];public static Vehicle[] NearbyVehicles=new Vehicle[0];public static Vehicle[] GetNearbyVehicles(Vector3 p,float r)=>NearbyVehicles;public static Prop CreateProp(Model m,Vector3 p,bool a,bool b)=>new Prop{Position=p};public static void AddExplosion(Vector3 v,ExplosionType t,float p,float q,Ped owner,bool a,bool b){}public static Camera RenderingCamera; public static List<Ped> Created=new List<Ped>();public static bool FailCamera;
  ...
  public static Vector3 GetNextPositionOnStreet(Vector3 p)=>p;public static float GroundHeight;public static float GetGroundHeight(Vector3 p)=>GroundHeight;
...
namespace GTA.Native {
 ...
 public static class Function {
  ...
  public static int Sprites, TankShots, DriveBys;public static bool ClearLos=true;public static bool InteriorReady=true;public static int InteriorId=123;public static int? EntityInterior;public static bool IplReady=true; public static bool ResetVitalsOnSwitch;public static bool EjectOnSwitch;public static Hash? ThrowOnce; public static Dictionary<Hash,object> Values=new Dictionary<Hash,object>();
...
if(h==Hash.SET_VEHICLE_IS_WANTED)((Vehicle)args[0]).IsWanted=(bool)args[1];if(ThrowOnce==h){ThrowOnce=null;throw new InvalidOperationException("Injected native failure");}if(h==Hash.ATTACH_VEHICLE_TO_TRAILER)Trailers[((Vehicle)args[0]).Handle]=(Vehicle)args[1];if(h==Hash.TASK_LEAVE_VEHICLE&&!((Ped)args[0]).StuckInSeat)((Ped)args[0]).CurrentVehicle=null;if(h==Hash.DRAW_SPRITE)Sprites++;if(h==Hash.SET_VEHICLE_SHOOT_AT_TARGET)TankShots++;if(h==Hash.TASK_DRIVE_BY)DriveBys++;if(args.Length>0)Values[h]=args[0];if(h==Hash.RESET_PLAYER_ARREST_STATE)Game.Arrested=false;if(h==Hash.CHANGE_PLAYER_PED){var old=Game.Player.Character;Game.Player.Character=(Ped)args[1];if(ResetVitalsOnSwitch){old.Health=200;old.Armor=0;old.MaxHealth=200;Game.Player.Character.Health=200;Game.Player.Character.Armor=0;Game.Player.Character.MaxHealth=200;}if(EjectOnSwitch)Game.Player.Character.Task.LeaveVehicle();}}
  public static T Call<T>(Hash h,params object[] args){if(ThrowOnce==h){ThrowOnce=null;throw new InvalidOperationException("Injected native failure");}object value=false;
...
namespace GTA.UI {
 public enum Alignment {Center,Right,Left} public class CustomSprite {public CustomSprite(string p,SizeF s,PointF f){}public void Draw(){}}
