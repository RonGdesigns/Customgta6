...
namespace GTA {
 ...
 public static class Game {public static int GenerateHash(string s){unchecked{uint h=0;foreach(char c in s.ToLowerInvariant()){h+=(byte)c;h+=h<<10;h^=h>>6;}h+=h<<3;h^=h>>11;h+=h<<15;return (int)h;}}public static float LastFrameTime=.016f;public static int GameTime; public static float TimeScale=1; public static Player Player=new Player(); public static bool Arrested,Accept,IsPaused; public static HashSet<Control> Disabled=new HashSet<Control>();public static void DisableControlThisFrame(Control c){Disabled.Add(c);} public static bool IsControlPressed(Control c)=>false;public static bool IsControlJustPressed(Control c){bool v=Accept;Accept=false;return v;}}
...
namespace GTA {
 ...
 public class Model {public int Hash=>Game.GenerateHash(Name);public bool IsInCdImage=true,IsValid=true;public bool IsSubmarine,IsTrain,IsBicycle;public bool IsCar=true,IsBike,IsHelicopter,IsPlane,IsBoat;public string Name;public Model(string s){Name=s;IsBoat=s=="longfin";IsHelicopter=s=="supervolito";IsPlane=s=="vestra";IsBike=s=="shinobi";IsCar=!IsBoat&&!IsHelicopter&&!IsPlane&&!IsBike;}public void MarkAsNoLongerNeeded(){}}
