...
namespace GTA.Math {
 public struct Vector3 { public float ToHeading()=>0f;
  ...
  public float DistanceTo2D(Vector3 v){return (float)System.Math.Sqrt((X-v.X)*(X-v.X)+(Y-v.Y)*(Y-v.Y));}
...
namespace GTA {
 ...
 public static class Game {public static InputMethod LastInputMethod=InputMethod.MouseAndKeyboard;public static int GenerateHash(string s){unchecked{uint h=0;foreach(char c in s.ToLowerInvariant()){h+=(byte)c;h+=h<<10;h^=h>>6;}h+=h<<3;h^=h>>11;h+=h<<15;return (int)h;}}public static float LastFrameTime=.016f;public static int GameTime; public static float TimeScale=1; public static Player Player=new Player(); public static bool Arrested,Accept,IsPaused; public static HashSet<Control> Disabled=new HashSet<Control>();public static void DisableControlThisFrame(Control c){Disabled.Add(c);} public static bool IsControlPressed(Control c)=>GTA.Native.Function.Held.TryGetValue(c,out var held)&&held;public static HashSet<Control> Pressed=new HashSet<Control>();public static bool IsControlJustPressed(Control c){if(Pressed.Remove(c))return true;bool v=Accept;Accept=false;return v;}}
 ...
 public class Ped:Entity {public PedBoneCollection Bones {get{Entity.BoneOwner=this;return new PedBoneCollection();}}public Model Model=new Model("ped");public bool IsCop;public int Health=300,MaxHealth=300;public bool CanSufferCriticalHits;public VehicleSeat SeatIndex {get {if(CurrentVehicle!=null)foreach(var p in CurrentVehicle.Seats)if(p.Value==this)return p.Key;return VehicleSeat.None;}}
  public bool DamageProof,DiesWhenInjured=true;public int MinStunGroundTime=-1;public uint LastWeaponHit;public bool IsRagdoll,IsCuffed,IsBeingStunned,CanBeDraggedOutOfVehicle,IsInWater;public DrivingStyle DrivingStyle; public void Kill(){IsDead=true;} public bool IsHeadtracking(Ped p)=>false;public bool IsAiming;public bool IsAlive=>!IsDead;public int Armor,Accuracy;public Weapons Weapons=new Weapons();public Blip AddBlip()=>new Blip();public void SetIntoVehicle(Vehicle v,VehicleSeat s){if(CurrentVehicle!=null)CurrentVehicle.Seats.Remove(SeatIndex);CurrentVehicle=v;Position=v.Position;v.Seats[s]=this;}
  public bool CanSwitchWeapons=true;public bool IsInCombat,IsShooting,AlwaysKeepTask,BlockPermanentEvents,Entering,Hostile,StuckInSeat,EjectOnTaskClear;
  public Vehicle CurrentVehicle; public int RelationshipGroup; public Ped CombatTarget;
  public Tasks Task; public Ped(){Task=new Tasks(this);} public bool IsSwimming,IsSwimmingUnderWater; public bool IsInVehicle()=>CurrentVehicle!=null;
 ...
 // A stand-in palette rather than the one paint the shop happened to name. The
 // sampler refuses a sweep that comes back with too few distinct colors, so a
 // one-entry enum would make that refusal untestable and the success path a lie.
 public enum VehicleColor {MetallicBlack,MetallicGraphiteBlack,MetallicSilver,MetallicBlueSilver,
  MetallicRed,MetallicTorinoRed,MetallicDarkGreen,MetallicRacingGreen,MetallicUltraBlue,
  MetallicBrightBlue,MetallicTaxiYellow,MetallicOrange,MetallicWhite,MatteBlack,HotPink,Blue,Orange}
 public enum ExplosionType {Grenade,Extinguisher,Boat,Tanker,Plane}
 public enum HeliMissionFlags {None} public enum VehicleMissionType {GoTo,Circle,Attack,Flee,Ram,Cruise,Escort} [System.Flags] public enum VehicleDrivingFlags {None=0,DrivingModeAvoidVehiclesReckless=1,AllowGoingWrongWay=2,UseShortCutLinks=4} public enum BoatMissionFlags {None}
 public class Tasks {public int VehicleMissions;public Entity MissionTarget;public Vector3 LastMissionPoint;public void StartVehicleMission(Vehicle v,Entity p,VehicleMissionType t,float s,VehicleDrivingFlags f,float r,float straight,bool wrong){VehicleMissions++;MissionTarget=p;}public void StartVehicleMission(Vehicle v,Vector3 p,VehicleMissionType t,float s,VehicleDrivingFlags f,float r,float straight,bool wrong){VehicleMissions++;LastMissionPoint=p;}public VehicleMissionType LastHeliMission;public void StartHeliMission(Vehicle v,Ped p,VehicleMissionType t,float s,float r,int h,int min,float a,float slow,HeliMissionFlags f){HeliTasks++;LastHeliMission=t;}public int Wanders,Chases;public void WanderAround(Vector3 p,float r){Wanders++;}public void VehicleChase(Ped p){Chases++;}public int VehicleShots;public void VehicleShootAtPed(Ped p){VehicleShots++;} public int Runs;public Vector3 LastRunPoint;public void RunTo(Vector3 p,bool a,int t){Runs++;LastRunPoint=p;}public void StartBoatMission(Vehicle v,Vehicle target,VehicleMissionType t,float s,VehicleDrivingFlags f,float r,BoatMissionFlags b){BoatTasks++;} public int HeliTasks;public void StartHeliMission(Vehicle v,Vector3 p,VehicleMissionType t,float s,float r,int h,int min,float a,float slow,HeliMissionFlags f){HeliTasks++;}public void StartPlaneMission(Vehicle v,Vector3 p,VehicleMissionType t,float s,float r,int h,int min,float a,bool b){}public int BoatTasks;public void StartBoatMission(Vehicle v,Vector3 p,VehicleMissionType t,float s,VehicleDrivingFlags f,float r,BoatMissionFlags b){BoatTasks++;LastMissionPoint=p;}public float LastDriveSpeed;public Vector3 LastDrivePoint;public int Drives,Gotos;public void DriveTo(Vehicle v,Vector3 p,float r,float s,DrivingStyle d){Drives++;LastDrivePoint=p;LastDriveSpeed=s;}public void LeaveVehicle(LeaveVehicleFlags f=LeaveVehicleFlags.None){if(owner.StuckInSeat)return;if(owner.CurrentVehicle!=null)owner.CurrentVehicle.Seats.Remove(owner.SeatIndex);owner.CurrentVehicle=null;}public void GoTo(Vector3 p){Gotos++;}public int Phones,Looks;public void UseMobilePhone(int ms){Phones++;}public void LookAt(Entity e,int ms){Looks++;}public void HandsUp(int t){}public void ChaseWithHelicopter(Ped p,Vector3 v){}public int Aims,Scenarios;public void AimAt(Ped p,int t){Aims++;}public float ScenarioHeading;public void StartScenario(string s,Vector3 p,float h){Scenarios++;ScenarioHeading=h;}public int Cruises;public void CruiseWithVehicle(Vehicle v,float f,DrivingStyle d){Cruises++;}public int HatedFights;public void FightAgainstHatedTargets(float r){HatedFights++;}
  readonly Ped owner; public int Clears,Enters,Warps,Fights; public VehicleSeat LastSeat; public Ped LastTarget;
  public Tasks(Ped p){owner=p;} public void StandStill(int t){} public void ClearAll(){Clears++;Playing=null;} public void ClearAllImmediately(){Clears++;Playing=null;if(owner.EjectOnTaskClear)LeaveVehicle();}public int Animations,Rappels;public string Playing;public void PlayAnimation(string d,string n,float a,float b,int t,AnimationFlags f,float r){Animations++;Playing=GTA.Native.Function.MissingAnimDicts.Contains(d)||GTA.Native.Function.SlowAnimDicts.Contains(d)||GTA.Native.Function.MissingClips.Contains(d+"|"+n)?null:d+"|"+n;}public int Flees;public void FleeFrom(Ped p){Flees++;}public void RappelFromHelicopter(){Rappels++;if(owner.CurrentVehicle!=null){owner.CurrentVehicle.Seats.Remove(owner.SeatIndex);owner.CurrentVehicle=null;}}
  ...
  public float LastEnterSpeed;public void EnterVehicle(Vehicle v,VehicleSeat s,int t=-1,float f=1f,EnterVehicleFlags flags=0){Enters++;LastEnterSpeed=f;LastSeat=s;owner.Entering=true;}
...
namespace GTA.Native {
 ...
 public static class Function {public static HashSet<GTA.Control> DisabledByOther=new HashSet<GTA.Control>();
  ...
  public static bool WantedSeen=true, StarsFlashing, ScoreAvailable=true, EaseHoldsView, Paused, MarineBlocked;
  ...
  /// <summary>Model ratings: estimated top speed in m/s, acceleration, braking, traction.
  /// Real enough to assert on; the game is the authority at runtime, as with the seat counts.</summary>
  public class ModelRating {public float Speed,Accel,Brake,Traction,BuiltAccel,BuiltBrake;}
  public static readonly Dictionary<string,ModelRating> ModelRatings=new Dictionary<string,ModelRating>(StringComparer.OrdinalIgnoreCase){
   {"granger",new ModelRating{Speed=42.5f,Accel=.32f,Brake=.72f,Traction=2.1f,BuiltAccel=.38f,BuiltBrake=.86f}},
   {"baller",new ModelRating{Speed=46.6f,Accel=.36f,Brake=.79f,Traction=2.25f,BuiltAccel=.42f,BuiltBrake=.93f}},
   {"benson",new ModelRating{Speed=30.2f,Accel=.18f,Brake=.55f,Traction=1.6f,BuiltAccel=.18f,BuiltBrake=.55f}},
  };
  /// <summary>When set, the model-rating natives answer zero until the model has streamed in.</summary>
  public static bool RatingsNeedModel;
  public static ModelRating Rating(int modelHash){
   foreach(var pair in ModelRatings) if(new GTA.Model(pair.Key).Hash==modelHash) return pair.Value;
   return new ModelRating{Speed=40f,Accel=.3f,Brake=.7f,Traction=2f,BuiltAccel=.3f,BuiltBrake=.7f};
  }
  public static int Seats(int modelHash){
   ...
   return 4;}
  /// <summary>Every paint index reading back the same color, the way a native that
  /// ignores the write would behave. The palette has to refuse that rather than draw
  /// a hundred and sixty identical tiles.</summary>
  /// <summary>Whether a started scenario is reported as playing. A scenario that will
  /// not take where a ped is standing is the case the posture verification exists for.</summary>
  public static bool ScenarioTakes=true;
  public static bool FlatPaint;
  ...
  /// <summary>Animation dictionaries the game does not have, ones still streaming, dictionary|clip pairs that do not take, and the dictionaries HAS_ANIM_DICT_LOADED reports resident (none unless a test says so).</summary>
  public static readonly HashSet<string> MissingAnimDicts=new HashSet<string>(),SlowAnimDicts=new HashSet<string>(),MissingClips=new HashSet<string>(),LoadedAnimDicts=new HashSet<string>();
  public static readonly List<Tuple<Hash,object[]>> Calls=new List<Tuple<Hash,object[]>>();public static void Call(Hash h,params object[] args){
   if(h==Hash.STOP_ANIM_TASK&&args[0] is Ped animated&&animated.Task.Playing==args[1]+"|"+args[2])animated.Task.Playing=null;
   if(h==Hash.TASK_PLAY_ANIM&&args[0] is Ped performer)performer.Task.PlayAnimation((string)args[1],(string)args[2],0f,0f,-1,default(AnimationFlags),0f);
...
Calls.Add(Tuple.Create(h,args));
   // Handing rendering back to gameplay is what this native does, interpolated or not.
   // A stand-in that ignored it would let a scene claim it had released the view while
   // still holding it.
   // The engine keeps rendering an eased hand-back's camera for the length of the slide;
   // EaseHoldsView models that window, which is where a scene started straight after
   // another one used to read the old camera as somebody else's view.
   // A vehicle blown up is a wreck; a velocity written is the one the entity reports.
   if(h==Hash.EXPLODE_VEHICLE&&args[0] is Vehicle blown)blown.IsDead=true;
   if(h==Hash.CLEAR_ENTITY_LAST_WEAPON_DAMAGE&&args[0] is GTA.Entity cleared)ExplosiveDamage.Remove(cleared.Handle);
   if(h==Hash.SET_ENTITY_VELOCITY&&args[0] is GTA.Entity moved)moved.Velocity=new Vector3((float)args[1],(float)args[2],(float)args[3]);
   if(h==Hash.RENDER_SCRIPT_CAMS&&args.Length>0&&args[0] is bool render&&!render&&!(EaseHoldsView&&args.Length>1&&args[1] is bool eased&&eased))World.RenderingCamera=null;
   if(args.Length>0&&args[0] is Vehicle cv){
    ...
    if(h==Hash.GET_VEHICLE_COLOURS){((OutputArgument)args[1]).Value=(int)cv.Mods.PrimaryColor;((OutputArgument)args[2]).Value=(int)cv.Mods.SecondaryColor;}
    if(h==Hash.SET_VEHICLE_COLOURS){cv.Mods.PrimaryColor=(VehicleColor)(int)args[1];cv.Mods.SecondaryColor=(VehicleColor)(int)args[2];}
    if(h==Hash.CLEAR_VEHICLE_CUSTOM_PRIMARY_COLOUR)cv.Mods.ClearCustomPrimaryColor();
    if(h==Hash.GET_VEHICLE_COLOR){Color paint=cv.Mods.IsPrimaryColorCustom?cv.Mods.CustomPrimaryColor:PaintFor((int)cv.Mods.PrimaryColor);
     ((OutputArgument)args[1]).Value=(int)paint.R;((OutputArgument)args[2]).Value=(int)paint.G;((OutputArgument)args[3]).Value=(int)paint.B;}
   ...
   if(h==Hash.SET_ENTITY_PROOFS){if(args[0] is Ped proofed)proofed.DamageProof=(bool)args[1];var flags=new bool[args.Length-1];for(int i=1;i<args.Length;i++)flags[i-1]=(bool)args[i];((Entity)args[0]).Proofs=flags;}
...
if(h==Hash.SET_VEHICLE_IS_WANTED)((Vehicle)args[0]).IsWanted=(bool)args[1];if(ThrowOnce==h){ThrowOnce=null;throw new InvalidOperationException("Injected native failure");}if(h==Hash.ATTACH_VEHICLE_TO_TRAILER)Trailers[((Vehicle)args[0]).Handle]=(Vehicle)args[1];if(h==Hash.TASK_LEAVE_VEHICLE&&!((Ped)args[0]).StuckInSeat)((Ped)args[0]).CurrentVehicle=null;if(h==Hash.DRAW_SPRITE)Sprites++;if(h==Hash.SET_VEHICLE_SHOOT_AT_TARGET)TankShots++;if(h==Hash.TASK_DRIVE_BY)DriveBys++;if(args.Length>0)Values[h]=args[0];if(h==Hash.RESET_PLAYER_ARREST_STATE)Game.Arrested=false;if(h==Hash.CHANGE_PLAYER_PED){var old=Game.Player.Character;Game.Player.Character=(Ped)args[1];if(ResetVitalsOnSwitch){old.Health=200;old.Armor=0;old.MaxHealth=200;Game.Player.Character.Health=200;Game.Player.Character.Armor=0;Game.Player.Character.MaxHealth=200;}if(EjectOnSwitch)Game.Player.Character.Task.LeaveVehicle();}}
  public static System.Collections.Generic.HashSet<int> TestDamage = new System.Collections.Generic.HashSet<int>();public static System.Collections.Generic.HashSet<int> ExplosiveDamage = new System.Collections.Generic.HashSet<int>();
  public static T Call<T>(Hash h,params object[] args){
   if(h==Hash.DOES_ANIM_DICT_EXIST)return (T)(object)!MissingAnimDicts.Contains((string)args[0]);
   if(h==Hash.HAS_ANIM_DICT_LOADED)return (T)(object)LoadedAnimDicts.Contains((string)args[0]);
   if(h==Hash.IS_ENTITY_PLAYING_ANIM)return (T)(object)(args[0] is Ped playing&&playing.Task.Playing==args[1]+"|"+args[2]);
   if(h==Hash.IS_CONTROL_ENABLED)return (T)(object)!DisabledByOther.Contains((GTA.Control)(int)args[1]);
   ...
   if(h==Hash.HAS_ENTITY_BEEN_DAMAGED_BY_WEAPON)return (T)(object)ExplosiveDamage.Contains(((GTA.Entity)args[0]).Handle);
...
if(ThrowOnce==h){ThrowOnce=null;throw new InvalidOperationException("Injected native failure");}object value=typeof(T)==typeof(bool)?(object)false:typeof(T)==typeof(string)?"":typeof(T)==typeof(float)?(object)0f:(object)0;
   ...
   if(RatingsNeedModel&&(h==Hash.GET_VEHICLE_MODEL_ESTIMATED_MAX_SPEED||h==Hash.GET_VEHICLE_MODEL_ACCELERATION||h==Hash.GET_VEHICLE_MODEL_MAX_BRAKING||h==Hash.GET_VEHICLE_MODEL_MAX_TRACTION||h==Hash.GET_VEHICLE_MODEL_ACCELERATION_MAX_MODS||h==Hash.GET_VEHICLE_MODEL_MAX_BRAKING_MAX_MODS||h==Hash.GET_VEHICLE_MODEL_NUMBER_OF_SEATS)&&!Model.LandedHashes.Contains(Convert.ToInt32(args[0])))return typeof(T)==typeof(int)?(T)(object)0:(T)(object)0f;
   ...
   if(h==Hash.GET_VEHICLE_MODEL_ESTIMATED_MAX_SPEED)return (T)(object)Rating(Convert.ToInt32(args[0])).Speed;
   if(h==Hash.GET_VEHICLE_MODEL_ACCELERATION)return (T)(object)Rating(Convert.ToInt32(args[0])).Accel;
   if(h==Hash.GET_VEHICLE_MODEL_MAX_BRAKING)return (T)(object)Rating(Convert.ToInt32(args[0])).Brake;
   if(h==Hash.GET_VEHICLE_MODEL_MAX_TRACTION)return (T)(object)Rating(Convert.ToInt32(args[0])).Traction;
   if(h==Hash.GET_VEHICLE_MODEL_ACCELERATION_MAX_MODS)return (T)(object)Rating(Convert.ToInt32(args[0])).BuiltAccel;
   if(h==Hash.GET_VEHICLE_MODEL_MAX_BRAKING_MAX_MODS)return (T)(object)Rating(Convert.ToInt32(args[0])).BuiltBrake;
   ...
   if(h==Hash.GET_VEHICLE_ESTIMATED_MAX_SPEED)return (T)(object)((Vehicle)args[0]).TopSpeed;
   if(h==Hash.IS_TURRET_SEAT)value=((Vehicle)args[0]).TurretSeats.Contains((VehicleSeat)(int)args[1]);
   if(h==Hash.HAS_COLLISION_LOADED_AROUND_ENTITY)value=World.CollisionReady;
   // Setting a vehicle on its wheels needs the ground under it to have streamed.
   if(h==Hash.SET_VEHICLE_ON_GROUND_PROPERLY)value=World.CollisionReady;
   if(h==Hash.CAN_ANCHOR_BOAT_HERE)value=!Values.TryGetValue(h,out var anchorable)||(bool)anchorable;
...
namespace Bloodlines.Crew {
 ...
 public class CompanionController {public Dictionary<CrewSlot,CrewOrder> Orders=new Dictionary<CrewSlot,CrewOrder>();public Dictionary<CrewSlot,Vehicle> OrderVehicles=new Dictionary<CrewSlot,Vehicle>();public CrewOrder OrderOf(CrewSlot s)=>Orders.TryGetValue(s,out var o)?o:CrewOrder.None;public bool HasOrders=>Orders.Count>0;public bool Order(CrewSlot s,CrewOrder o,Vehicle v){if(MissionActive||RequireSharedVehicle||Controlled.Contains(s))return false;if(CrewOrders.AboutVehicle(o)&&v==null)return false;if(o==CrewOrder.OwnThing){Orders.Remove(s);return SetHangout(s,false);}SetHangout(s,true);if(o==CrewOrder.FollowMe||o==CrewOrder.None){Orders.Remove(s);return true;}Orders[s]=o;OrderVehicles[s]=v;return true;}public void ClearOrder(CrewSlot s){Orders.Remove(s);}public void ClearOrders(){Orders.Clear();}public Dictionary<CrewSlot,bool> TravelChoices=new Dictionary<CrewSlot,bool>();public bool RidesAlong(CrewSlot s)=>TravelChoices.ContainsKey(s)?TravelChoices[s]:RideAlong;public bool SetTravelChoice(CrewSlot s,bool ride){if(!SetHangout(s,true))return false;TravelChoices[s]=ride;return true;}public Dictionary<CrewSlot,bool> Hangouts=new Dictionary<CrewSlot,bool>();public bool HasIndividualOrders=>Hangouts.Count>0;public bool IsHangingOut(CrewSlot s)=>Hangouts.ContainsKey(s)?Hangouts[s]:!IndependentFreeRoam;public bool SetHangout(CrewSlot s,bool value){if(MissionActive||Controlled.Contains(s))return false;Hangouts[s]=value;return true;}public bool IndependentFreeRoam=true,RideAlong=true;public int Refreshes;public void Refresh(CrewSlot slot){Refreshes++;}public bool MissionActive;public CompanionLife Life=new CompanionLife();public CompanionPresence Presence=new CompanionPresence();public bool RequireSharedVehicle;public bool StunGunOnly;public void ReleaseAll(){StunGunOnly=false;Rejoining.Clear();Controlled.Clear();Presence.Clear();RequireSharedVehicle=false;}public CompanionState StateOf(CrewSlot s)=>Controlled.Contains(s)?CompanionState.Scripted:CompanionState.Follow;public System.Collections.Generic.HashSet<CrewSlot> Controlled=new System.Collections.Generic.HashSet<CrewSlot>();public void TakeControl(CrewSlot s){Rejoining.Remove(s);Controlled.Add(s);Presence.Release(s);}public System.Collections.Generic.HashSet<CrewSlot> Rejoining=new System.Collections.Generic.HashSet<CrewSlot>();public void Rejoin(CrewSlot s){Controlled.Remove(s);Rejoining.Add(s);}public bool IsRejoining(CrewSlot s)=>Rejoining.Contains(s);public void ReleaseControl(CrewSlot s){Rejoining.Remove(s);Controlled.Remove(s);Presence.Release(s);}public bool IsHeld(CrewSlot s)=>Controlled.Contains(s);}
...
namespace GTA {
 public enum InputMethod {MouseAndKeyboard,GamePad}
 ...
 public class Model {public static bool operator==(Model a,Model b)=>ReferenceEquals(a,b)||(!(a is null)&&!(b is null)&&a.Name==b.Name);public static bool operator!=(Model a,Model b)=>!(a==b);public override bool Equals(object o)=>o is Model m&&m==this;public override int GetHashCode()=>Name.GetHashCode();public int Hash=>Game.GenerateHash(Name);public bool IsInCdImage=true,IsValid=true;public (Vector3 rearBottomLeft,Vector3 frontTopRight) Dimensions=>Name=="lazer" ? (new Vector3(-5f,-7.5f,-1f),new Vector3(5f,7.5f,3f)) : Name=="avenger" ? (new Vector3(-16.10f,-11.66f,-2.52f),new Vector3(16.14f,10.49f,6.04f)) : Name=="duster" ? (new Vector3(-7.5f,-5f,-1.2f),new Vector3(7.5f,5f,2.5f)) : (Name=="granger"||Name=="baller") ? (new Vector3(-1f,-2.5f,-.6f),new Vector3(1f,2.5f,1.7f)) : Name=="tug"?(new Vector3(-5.141756f,-16.78415f,-3.719979f),new Vector3(5.141756f,14.29196f,10.97245f)):(new Vector3(-0.6f,-0.4f,0f),new Vector3(0.6f,0.4f,Name=="prop_table_03"?0.75f:0.05f));public bool IsSubmarine,IsTrain,IsBicycle;public bool IsCar=true,IsBike,IsHelicopter,IsPlane,IsBoat;public bool IsVehicle=>IsCar||IsBike||IsBoat||IsHelicopter||IsPlane||IsSubmarine;public static bool StreamsOnRequest=true;/* The engine streams a model over the frames after a request, and a release before it lands cancels it. */public static bool StreamsNextFrame;public static readonly HashSet<string> Pending=new HashSet<string>();public static readonly HashSet<string> Landed=new HashSet<string>();public static readonly HashSet<int> LandedHashes=new HashSet<int>();public static void Stream(){foreach(var name in Pending){Landed.Add(name);LandedHashes.Add(Game.GenerateHash(name));}Pending.Clear();}private bool _loaded;public bool IsLoaded{get{return _loaded||Landed.Contains(Name);}private set{_loaded=value;}}public void Request(){if(StreamsNextFrame){Pending.Add(Name);return;}IsLoaded=StreamsOnRequest;}public string Name;public Model(int hash){Name="model#"+hash;}public Model(string s){Name=s;IsSubmarine=s=="submersible2";IsBoat=s=="longfin"||s=="tug"||s=="tropic"||s=="dinghy";IsHelicopter=s=="supervolito"||s=="cargobob"||s=="polmav";IsPlane=s=="vestra";IsBike=s=="shinobi";IsCar=!IsBoat&&!IsHelicopter&&!IsPlane&&!IsBike&&!IsSubmarine;}public void MarkAsNoLongerNeeded(){if(StreamsNextFrame)Pending.Remove(Name);}}
...
namespace Bloodlines.Missions {
 ...
 public class MissionDefinition {public Core.MissionInfo Info;public System.Func<Mission> Factory;public bool IsPlayable=true;public bool IsSolo=>Info.IsSolo;public string Id=>Info.Id;public string Title=>Info.Title;public Act Act=>Act.I;public static string ActTitle(Act a)=>"Act I — The Bleeding Trail";}
