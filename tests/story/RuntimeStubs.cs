...
namespace GTA {
 ...
 public static class Game {public static int GenerateHash(string s){unchecked{uint h=0;foreach(char c in s.ToLowerInvariant()){h+=(byte)c;h+=h<<10;h^=h>>6;}h+=h<<3;h^=h>>11;h+=h<<15;return (int)h;}}public static float LastFrameTime=.016f;public static int GameTime; public static float TimeScale=1; public static Player Player=new Player(); public static bool Arrested,Accept,IsPaused; public static HashSet<Control> Disabled=new HashSet<Control>();public static void DisableControlThisFrame(Control c){Disabled.Add(c);} public static bool IsControlPressed(Control c)=>false;public static HashSet<Control> Pressed=new HashSet<Control>();public static bool IsControlJustPressed(Control c){if(Pressed.Remove(c))return true;bool v=Accept;Accept=false;return v;}}
 ...
 public class Ped:Entity {public PedBoneCollection Bones {get{Entity.BoneOwner=this;return new PedBoneCollection();}}public Model Model=new Model("ped");public bool IsCop;public int Health=300,MaxHealth=300;public bool CanSufferCriticalHits;public VehicleSeat SeatIndex {get {if(CurrentVehicle!=null)foreach(var p in CurrentVehicle.Seats)if(p.Value==this)return p.Key;return VehicleSeat.None;}}
  ...
  public bool IsInCombat,IsShooting,AlwaysKeepTask,BlockPermanentEvents,Entering,Hostile,StuckInSeat;
 ...
 public enum BlipSprite {Tank,PoliceCarDot,PoliceHelicopter,Safehouse,Helicopter,Plane,Boat,Standard,Enemy,PersonalVehicleCar,ArmoredTruck} public enum BlipColor {Yellow,Blue,Green,Orange,Purple,Red}
 ...
 public static class World {public static Func<Vector3,Vector3,RaycastResult> RaycastHandler;public static RaycastResult Raycast(Vector3 s,Vector3 t,IntersectFlags f,Entity e=null)=>RaycastHandler==null?new RaycastResult():RaycastHandler(s,t);public static Vehicle[] GetAllVehicles()=>Vehicles.ToArray();public static Prop[] GetNearbyProps(Vector3 p,float r)=>new Prop[0];public static Vehicle[] NearbyVehicles=new Vehicle[0];public static Vehicle[] GetNearbyVehicles(Vector3 p,float r)=>NearbyVehicles;public static List<Prop> Props=new List<Prop>();public static Prop CreateProp(Model m,Vector3 p,bool a,bool b){var prop=new Prop{Position=p,Model=m};Props.Add(prop);return prop;}public static void AddExplosion(Vector3 v,ExplosionType t,float p,float q,Ped owner,bool a,bool b){}public static Camera RenderingCamera; public static List<Ped> Created=new List<Ped>();public static bool FailCamera;
  ...
  public static TimeSpan CurrentTimeOfDay=TimeSpan.FromHours(13);public static Vector3 GetNextPositionOnStreet(Vector3 p)=>p;public static float GroundHeight;public static float GetGroundHeight(Vector3 p)=>GroundHeight;
  public static Vector3? FailNavigationNear;public static Vector3 GetSafeCoordForPed(Vector3 p,bool sidewalk,int flags)=>FailNavigation||(FailNavigationNear.HasValue&&p.DistanceTo(FailNavigationNear.Value)<30f)?Vector3.Zero:p;
...
namespace GTA.Native {
 ...
 public static class Function {
  ...
  public static int Sprites, TankShots, DriveBys;public static bool ClearLos=true;public static bool InteriorReady=true;public static int InteriorId=123;public static int? EntityInterior;public static bool IplReady=true;public static bool InteriorDisabled,InteriorCapped,StarsGreyed; public static bool ResetVitalsOnSwitch;public static bool EjectOnSwitch;public static Hash? ThrowOnce;public static float Seabed=-40f;public static bool SeabedKnown=true; public static Dictionary<Hash,object> Values=new Dictionary<Hash,object>();
  public static readonly List<Tuple<Hash,object[]>> Calls=new List<Tuple<Hash,object[]>>();public static void Call(Hash h,params object[] args){Calls.Add(Tuple.Create(h,args));
...
if(h==Hash.SET_VEHICLE_IS_WANTED)((Vehicle)args[0]).IsWanted=(bool)args[1];if(ThrowOnce==h){ThrowOnce=null;throw new InvalidOperationException("Injected native failure");}if(h==Hash.ATTACH_VEHICLE_TO_TRAILER)Trailers[((Vehicle)args[0]).Handle]=(Vehicle)args[1];if(h==Hash.TASK_LEAVE_VEHICLE&&!((Ped)args[0]).StuckInSeat)((Ped)args[0]).CurrentVehicle=null;if(h==Hash.DRAW_SPRITE)Sprites++;if(h==Hash.SET_VEHICLE_SHOOT_AT_TARGET)TankShots++;if(h==Hash.TASK_DRIVE_BY)DriveBys++;if(args.Length>0)Values[h]=args[0];if(h==Hash.RESET_PLAYER_ARREST_STATE)Game.Arrested=false;if(h==Hash.CHANGE_PLAYER_PED){var old=Game.Player.Character;Game.Player.Character=(Ped)args[1];if(ResetVitalsOnSwitch){old.Health=200;old.Armor=0;old.MaxHealth=200;Game.Player.Character.Health=200;Game.Player.Character.Armor=0;Game.Player.Character.MaxHealth=200;}if(EjectOnSwitch)Game.Player.Character.Task.LeaveVehicle();}}
  public static T Call<T>(Hash h,params object[] args){
   if(h==Hash.PREPARE_MUSIC_EVENT||h==Hash.TRIGGER_MUSIC_EVENT||h==Hash.CANCEL_MUSIC_EVENT){Calls.Add(Tuple.Create(h,args));if(ThrowOnce==h){ThrowOnce=null;throw new InvalidOperationException("Injected audio failure");}return (T)(object)ScoreAvailable;}
   ...
   if(h==Hash.GET_SHAPE_TEST_RESULT){
    ...
    ((OutputArgument)args[1]).Value=vertical||MarineBlocked;
    ((OutputArgument)args[2]).Value=new Vector3((float)LastRay[0],(float)LastRay[1],Seabed);
    return (T)(object)2;
   }
if(ThrowOnce==h){ThrowOnce=null;throw new InvalidOperationException("Injected native failure");}object value=false;
...
namespace GTA.UI {
 public enum Font {ChaletLondon,Pricedown=7} public enum Alignment {Center,Right,Left} public class CustomSprite {public CustomSprite(string p,SizeF s,PointF f){}public void Draw(){}}
 ...
 public class TextElement {public Alignment Alignment;public Font Font;public TextElement(string t,PointF p,float s,Color c){}public void Draw(){}}
}
