...
public static partial class StoryTests
{
 static void ApartmentWeaponChecks()
 {
  ...
  Function.EntityInterior=0;access.Begin(inside,"apa_v_mp_h_01_c",true);Game.GameTime+=300;access.Update();access.Update();
  Check(access.Inside&&!access.Busy&&Function.Values.ContainsKey(Hash.CLEAR_ROOM_FOR_ENTITY),"Ready apartment does not roll back solely because a frozen player's room association has not updated");access.Cancel();
  Function.EntityInterior=999;access.Begin(inside,null,true);Game.GameTime+=300;access.Update();access.Update();
  Check(access.Busy&&!access.Inside,"A mismatched nonzero interior still blocks unsafe entry");access.Cancel();Function.EntityInterior=null;
  Function.IplReady=false;access.Begin(inside,"apa_v_mp_h_01_c",true);Game.GameTime+=300;access.Update();
  Check(access.Busy&&player.Position==before,"Luxury entry waits for its requested IPL before moving the hero");Function.IplReady=true;access.Update();access.Update();
  Check(access.Inside&&!access.Busy,"Luxury entry completes once the requested IPL and collision are ready");access.Cancel();
