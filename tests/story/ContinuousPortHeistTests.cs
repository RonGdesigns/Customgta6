...
public static partial class StoryTests
{
    ...
    static void RunWholePortAttempt(string stopAt, string exitKind)
    {
        ...
        Check(!c.Cutscenes.IsActive, "M20 opens inside the operation with no scene: the action does not stop at the join");
        ...
        Check(!m20.Transferred && !c.Cutscenes.IsActive && m20.CurrentStage == 4, "The launch transfer runs live: no scene, the crew boards under AI while Ron holds the lift");
        for (int i = 0; i < 8 && !m20.Transferred; i++) { Game.GameTime += 16000; c.Dialogue.Clear(); m20.Tick(); }
        ...
        if (StopWholePortAttempt(manager, c, state, catalog, stopAt, exitKind, savePath)) return;
        Check(!c.Cutscenes.IsActive, "M21 opens inside the operation with no scene");
        Use(crew, CrewSlot.Gohan); m21.Tick();
        ...
        Check(!c.Cutscenes.IsActive && !m21.Transferred && m21.CurrentStage == 5, "Shore arrival opens the road pickup live: no cut, Gohan walks to the Granger himself");
        Game.Player.Character.SetIntoVehicle(m21.Granger, VehicleSeat.Driver);
        for (int i = 0; i < 8 && !m21.Transferred; i++) { Game.GameTime += 16000; c.Dialogue.Clear(); m21.Tick(); }
        Check(m21.Transferred && PortHeistWorld.Seated(actors[CrewSlot.Gohan], m21.Granger, VehicleSeat.Driver) && PortHeistWorld.Seated(actors[CrewSlot.Ice], m21.Granger, VehicleSeat.Passenger), "Ice boards the Granger under AI beside Gohan; only real seats commit the road transfer");
        var roadCar = m21.Granger; var coastPosition = roadCar.Position; var airPosition = helicopter.Position;
        ...
        Check(!c.Cutscenes.IsActive && !m22.Arrived && !m22.RoadArrivalPending, "Reaching the Alamo starts the road arrival live: no cut while Ron is flying");
