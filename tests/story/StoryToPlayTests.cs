...
public static partial class StoryTests
{
 ...
 static void RunM04(bool killMiller)
 {
  ...
  Check(van!=null&&van.Model.Name=="granger"&&guess.CurrentVehicle==van&&ice.CurrentVehicle==null&&gohan.CurrentVehicle==null&&ice.Position.DistanceTo(m4.Roles.For(CrewSlot.Ice).Point)<3f&&m4.Roles.For(CrewSlot.Ice).State==RoleState.Observing&&Game.Player.Character==guess,"Ron leaves the base alone in the crew van; Ice and Gohan are already on the lot in position, never seen arriving");
  ...
  Check(c.Dialogue.HasPending,"Halfway there Gohan calls that he is already on the breaker");
  guess.Position=exit;van.Position=exit;van.Speed=0f;m4.Tick();m4.Tick();
  Check(m4.CurrentStage>=1&&ice.CurrentVehicle==null&&gohan.CurrentVehicle==null,"Arrival drops nobody off: Guess holds the exit while the others are already in place");
  m4.Tick();m4.Tick();
