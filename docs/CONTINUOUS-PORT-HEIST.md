...
Comparison branch: `codex/continuous-port-heist`. Do not merge without review and the live checks below.
...
A save already between the old independent chapters resumes at the first incomplete phase. A bookmark from this version takes precedence for a new unfinished operation. A save already past M22 is never forced backward. The normal map presents one operation entry, labelled with the resume phase where applicable; the developer catalog retains individual phases.
