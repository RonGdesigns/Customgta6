...
- Nearby switches, within 80 meters in three dimensions, transfer control immediately. Distant switches use a 150 ms fade plus at most 1.5 seconds of collision checks, then a 200 ms fade-in. Unready collision leaves the existing hero active. These remove the old scripted sky-camera waits; actual streaming performance still depends on GTA and the machine.
