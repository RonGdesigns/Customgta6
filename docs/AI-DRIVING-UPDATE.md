# AI, driving and mission update — September 8, 2026
...
- Switching away from a driver transfers driving to shared AI. Assigned mission destinations take precedence over a map waypoint; without either, the driver cruises cautiously or follows the traveling crew. Mission-owned chases take precedence over generic driving.
...
- M02 starts Guess driving, with Gohan and Ice already seated. Catch the moving van, then remain within **35 meters** for **24 seconds of connected hacking**. You may keep driving as Guess or switch to passenger Gohan; leaving range pauses the hack.
...
2. Leave a companion on foot, drive past 200 meters and then 500 meters. Check normal acquisition of a separate car, followed by unseen catch-up if necessary. Look back deliberately: no recovery should appear in view. Toggle Independent and confirm the crew stays local instead.
3. Replay M02 from the start. Test hacking while playing Guess, then Gohan; briefly exceed 35 meters; watch for shooting at halfway. Retrieve the drives as Ice, reboard and finish at the canal. Inspect the current-objective debug page if a stage is unclear.
