...
Per vehicle instance (current car only, frame-safe): `ApplyForce` (`APPLY_FORCE_TO_ENTITY`), `SET_VEHICLE_REDUCE_GRIP`, `SET_VEHICLE_CHEAT_POWER_INCREASE`, `SET_VEHICLE_MAX_SPEED`. **Correction from implementation (September 10):** there is no per-vehicle gravity amount in 3.6 (`Vehicle.Gravity` does not exist; the assembly has only `HasGravity` and world gravity), so the weight term in §2.3 is applied as a bounded downward force instead.
...
- A constant quarter-g weight press while the car is on all its wheels (implemented as force, see the §2.1 correction: the planned gravity ×1.25 has no per-vehicle setter in 3.6; ramps and jumps unaffected because the check drops it airborne).
- Bounded downforce: `ApplyForce` straight down, proportional to speed squared, capped at 0.6 g equivalent, only while grounded and upright, ramped in over 0.5 s and out over 0.5 s so the ability ending mid-corner does not drop the car. Weight and downforce together never exceed 0.85 g.
