...
def main():
    ...
    parser.add_argument('--fix', action='store_true',
                        help='rewrite wrong-district coordinates to the correct zone centre')
    args = parser.parse_args()

    zones = load_zones()
    with open(LOCATIONS) as handle:
    ...
    for row in rows:
        ...
        # neighbour and overlap, and the bible names places loosely.
        ...
        if args.fix and expected and issues:
            ...
            row['status'] = 'zone-centre'

    if args.fix:
        with open(LOCATIONS, 'w') as handle:
            cols = ['key', 'x', 'y', 'z', 'heading', 'kind', 'status', 'district_hint']
            handle.write('\t'.join(cols) + '\n')
            for row in rows:
                handle.write('\t'.join(row[c] for c in cols) + '\n')
        print('rewrote {} of {} coordinates to their zone centre'.format(
            sum(1 for r in rows if r['status'] == 'zone-centre'), len(rows)))
