    ...
    public sealed class M39ThePaletoCable : CoastalOperation
    {
        ...
        protected override IEnumerable<MissionStage> BuildStages()
        {
            ...
            yield return new MissionStage("Clamp the junction",new MissionInteraction("Gohan: stop the sub beside the marked cable junction and fit its cutter clamp",()=>At("M39.CutterWork"),6,7f,()=>Sub,true)).OwnedBy(CrewSlot.Gohan)
                .OnExit(c=>{_clamp=WorkProp("prop_ld_bomb_01",_junction.Position,false);if(!RequireAssets(_clamp))throw new InvalidOperationException("Cable cutter could not load.");SaveCargo(_clamp,_junction,new Vector3(0,-.6f,.6f));RequireAsset(_clamp,"The required cable cutter was destroyed.");Establish("clamp","Cutter attached","The cutter is attached to the visible junction. Gohan must run and verify it before calling the link severed.",_junction);});
            yield return new MissionStage("Run and verify cutter",new MissionInteraction("Gohan: hold the sub beside the junction while the cutter runs and verify the link is down",()=>At("M39.CutterWork"),8,7f,()=>Sub,true)).OwnedBy(CrewSlot.Gohan)
                .OnExit(c=>{if(!Attached(_clamp,_junction))throw new InvalidOperationException("The cutter detached before completion.");_cut=true;for(int i=1;i<=4;i++)Enemy("M39.Guard"+i);Fighting=true;}).AfterCues("M39_S1_01_GOHAN");
