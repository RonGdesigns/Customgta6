...
namespace Bloodlines.Missions.Campaign
{
    public sealed class M31TheIronPerimeter : PreparationOperation
    {
        ...
        protected override bool Setup()
        {
            ...
            CrewCar = CrewTransport("M31.CrewCar");
            _generator = Equipment("prop_generator_03b", "M31.Generator");
            RequireAsset(_generator, "The perimeter generator was destroyed. Restart the defense.");
            for (int i = 1; i <= 3; i++) _barriers.Add(Equipment("prop_barrier_work05", "M31.Barrier" + i));
        ...
        {
            yield return new MissionStage("Read the approaches", new MultiHoldObjective("Ice: inspect each yellow approach marker beside the three barriers; press E / D-pad Right", Enumerable.Range(1,3).Select(i=>At("M31.Work"+i)),2,3f,"Checking the approach")).OwnedBy(CrewSlot.Ice);
            yield return new MissionStage("Arm the perimeter", new MultiHoldObjective("Gohan: arm each marked barrier charge; press E / D-pad Right",Enumerable.Range(1,3).Select(i=>At("M31.Work"+i)),4,3f,"Arming charge") { Animation=MissionInteraction.ReachInside, SiteDone=i=>Arm(i) }).OwnedBy(CrewSlot.Gohan)
                .OnEnter(c=>Roles.For(CrewSlot.Ice).Observe(At("M31.IceStart"),At("M31.IceStart"))).AfterCues("M31_S1_01_ICE");
            yield return new MissionStage("Prove the withdrawal road",new TravelObjective("Guess: drive the crew car to the yellow withdrawal marker and stop. Leave the road clear.",()=>At("M31.Retreat"),12,()=>CrewCar)).OwnedBy(CrewSlot.Guess)
                .OnExit(c=>StartProbe());
            yield return new MissionStage("Hold the perimeter",new KillTargetsObjective("Defend the generator. Stop the marked probe convoy; planted charges fire only when enemies enter their lane and the crew is clear.",()=>Opposition))
                .WithCues("M31_S1_02_GOHAN").OnEnter(c=>{Roles.For(CrewSlot.Guess).Stop();Roles.For(CrewSlot.Ice).TakeCover(At("M31.IceStart"));Roles.For(CrewSlot.Gohan).TakeCover(At("M31.GohanStart"));})
                .AfterCues("M31_S1_03_ICE");
            yield return new MissionStage("Check the damage",new MissionInteraction("Gohan: inspect the surviving generator controls",()=>At("M31.GeneratorWork"),4,3f,animation:MissionInteraction.ReachInside,face:()=>_generator.Position)).OwnedBy(CrewSlot.Gohan).OnEnter(c=>Fighting=false);
        ...
        {
            var charge=Equipment("prop_ld_bomb_01","M31.Charge"+(i+1));_charges.Add(charge);ArmedCount++;
        }
        private void StartProbe()
        {
            if(ArmedCount!=3)throw new InvalidOperationException("The perimeter has not been armed.");
            _probeStarted=true;Fighting=true;
            ResponseCar("M31.Convoy1",At("M31.Barrier1"));ResponseCar("M31.Convoy2",At("M31.Barrier2"));
