...
namespace Bloodlines.Missions.Campaign
{
    public sealed class M40ThePhantomRigging : CoastalOperation
    {
        ...
        protected override bool Setup()
        {
            ...
            Establish("approach","Build at the open cove","Two Tropics lie alongside the cove pier's north moorings, reachable on foot from the deck. Guess prepares the hull kits, Ice tests carried weapons on the marked floating targets, and Gohan verifies the route at a real laptop. No walkable cave or mounted turret is implied.",_boats[0],_kits[0],_nav);
            return true;
        }
        protected override IEnumerable<MissionStage> BuildStages()
        {
            ...
            for(int i=0;i<2;i++)
            {
                ...
                yield return new MissionStage("Trial boat "+(n+1),new EnterVehicleObjective(Protagonist.Of(slot).DisplayName+": walk out along the pier and board moored boat "+(n+1)+" as driver",()=>_boats[n],VehicleSeat.Driver)).OwnedBy(slot);
                ...
                yield return new MissionStage("Return boat "+(n+1),new TravelObjective("Return the same boat to its yellow mooring beside the pier and stop",()=>At("M40.Return"+(n+1)),10f,()=>_boats[n])).OwnedBy(slot).OnExit(c=>{_trials++;Roles.For(slot).Stop();});
            }
