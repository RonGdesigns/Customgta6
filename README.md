...
- [x] Expand high-demand vehicle catalog to 202 distinct models across 10 transport categories, adding dedicated `Weaponized` and `Electric` pools, Tuners, Drift icons, Stunt/Special cars, and Interceptors with dynamic specs and dealership integration (`StoryVehicles.cs`, `Garages.cs`, `ExpansionTests.cs`) (2026-09-16)
...
- **2026-09-16**: Expanded native vehicle catalog from 118 to 202 distinct models across 10 transport categories. Added dedicated `Weaponized` pool (27 vehicles: Imani Tech armed cruisers, Deluxo, Stromberg, Toreador, Scramjet, Vigilante, Ruiner 2000, TM-02 Khanjali railgun tank, APC, Barrage, Oppressors) and dedicated `Electric` pool (13 EV vehicles: Buffalo EVX, Coil Cyclone I & II, Tezeract, Raiden, Neon, I-Wagen, Powersurge). Added JDM/Tuners and Drift icons (`sultan3`, `rt3000`, `driftfr36`, `vorschlaghammer`, `drifttampa`), Stunt/Special vehicles (Ramp Buggy, Phantom Wedge, F1 PR4/BR8), Monster Trucks, and Police Interceptors. Fully verified via Roslyn compiler, 4,662 story tests, 206 regression checks, and mission linter (zero errors).
...
- **2026-09-16 — Dedicated Weaponized & Electric Vehicle Categories**: Per user direction, separated vehicles capable of equipping or firing weapons (Imani Tech, roof turrets, missile racks, torpedoes, and cannons) into a dedicated `Weaponized` category, and established an `Electric` category for EV performance and hypercars. This preserves `Cars` exclusively for pure combustion tuners, drift icons, and classic muscle without weapon clutter, expanding the system to 10 distinct transport categories.
